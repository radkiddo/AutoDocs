    ...
    mytables = (function () {
    	var 
    		...
    		_db = {
    			...
    			itemDeleteExt : function (table, itemName, item, itemNameExt, itemExt, sParams, fParams, onFound, onNotFound) {
    				if (_domainPath === undefined || _appName === undefined || _domainPath === '' || _appName === '') {
    					onNotFound(fParams, 'itemDeleteExt = empty domainPath or appName');
    					return;
    				}

    				if (table === undefined || table === '' || item === undefined || item === '' || itemName === undefined || itemName === '') {
    					onNotFound(fParams, 'itemDeleteExt = empty table, itemName or item');
    					return;
    				}

    				if (table.indexOf(' ') >= 0 || itemName.indexOf(' ') >= 0 || item.indexOf(' ') >= 0) {
    					onNotFound(fParams, 'itemDeleteExt = table, itemName, item cannot contain spaces');
    					return;
    				}

    				if (table.indexOf(' ') >= 0 || itemNameExt.indexOf(' ') >= 0 || itemExt.indexOf(' ') >= 0) {
    					onNotFound(fParams, 'itemDeleteExt = table, itemNameExt, itemExt cannot contain spaces');
    					return;
    				}
   							...
   							if (data !== null && data !== undefined) {
   								if (data.length > 0) {
   									...
   									for (var i = 0; i <= data.length - 1; i++) {
   										...
   										if (_db.hasOwnValueStr(obj, itemName, item) &&
   											_db.hasOwnValueStr(obj, itemNameExt, itemExt)) {
   											$.ajax({
   												type: 'GET',
   												url: _domainPath + '/' + 'deleterow' + '/' + _appName + '_' + table + '/' + di
   													 + '/' + _s3aKy + '/' + _s3stKy + '/' + _s3rg,
   												success: function (result) {
   													onFound(result, sParams, 'itemDeleteExt = deleted');
   													found = true;
   												},
   												error: function (XMLHttpRequest, textStatus, errorThrown) {
													onNotFound(fParams, 'itemDeleteExt = failed: ' + textStatus + ' -> ' + errorThrown);
   												}
   											});
   										}
   									}
   									...
   									if (!found) {
   										onNotFound(fParams, 'itemDeleteExt = no elements in returned result');
   									}
   								}
   								else {
   									onNotFound(fParams, 'itemDeleteExt = no elements in returned result');
   								...
   								onNotFound(fParams, 'itemDeleteExt = null or undefined return value');
   							}
   						},
   						error: function (XMLHttpRequest, textStatus, errorThrown) {
							onNotFound(fParams, 'itemDeleteExt = failed: ' + textStatus + ' -> ' + errorThrown);
